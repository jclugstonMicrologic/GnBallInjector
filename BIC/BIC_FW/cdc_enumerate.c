...
static uint AT91F_UDP_Read(AT91PS_CDC pCdc, char *pData, uint length)
{
	...
	while (length)
	{
// I do not know why the break statement below is not being executed when the USB is disconnected
// non the less this works (takes about 4 seconds to get here, must be the while(packetSize--)	
// that is holding us up.  The 4 seconds is actually the WatchDog timing out, so KickWdt()
// below is never being invoked
